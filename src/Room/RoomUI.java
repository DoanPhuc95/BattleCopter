...
import battlecopter_project.RoomManager;
...
public class RoomUI extends JFrame {
    ...
    private JComboBox<String> selectLv = new JComboBox<>();
	/**
    ...
    public RoomUI(Client_server client, int color, int lv, boolean isBoss)
    {
        this(client,color,isBoss);
        updateLevel(lv);
    }
    private void init()
    {
        ...
        selectLv.setModel(new DefaultComboBoxModel<>(new String[]{"Level 1","Level 2","Level 3","Level 4"}));
        selectLv.setBounds(230, 50, 150, 50);add(selectLv);
    ...
    {
        selectLv.setSelectedItem(selectLv.getModel().getElementAt(lv));
        repaint();
    }
    ...
    public void PlayGame(ObjectLocation ol)
    {
        ...
        new gameplay.Game(client,color,client.getName(),ol).setVisible(true);
        this.setVisible(false);
    }
    
    private void setLevel()
    {
        if(!selectLv.isEnabled()) return;
        client.getTransferMethod().SEND("LEVEL");
        client.getTransferMethod().SEND(selectLv.getSelectedIndex());
    }
}
